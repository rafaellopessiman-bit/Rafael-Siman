...
describe('Ask Surface (smoke e2e)', () => {
  ...
  it('should answer with citations for grounded ask requests', async () => {
    await request(app.getHttpServer())
      .post('/knowledge')
      .send({
        sourceFile: 'guia-atlas.md',
        content: 'Atlas Local oferece busca semantica e runtime agentico para documentos.',
      })
      .expect(201);
    ...
    expect(response.body).toHaveProperty('grounded', true);
    expect(Array.isArray(response.body.citations)).toBe(true);
    expect(response.body.citations[0]).toHaveProperty('sourceFile', 'guia-atlas.md');
  });
});