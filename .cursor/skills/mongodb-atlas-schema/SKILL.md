...
description: >
  Fornece templates e melhores práticas de schema design para MongoDB Atlas Local com
  @nestjs/mongoose. Use ao modelar entidades, índices ou padrões de embedding/referencing.
paths: "**/*.ts"
---

# MongoDB Atlas Schema — NestJS/Mongoose
...
```bash
# Nunca hardcode a URI — leia de .env (ver .env.example)
mongosh "$MONGODB_URI"
```
...
- ❌ Datas como strings ISO — use sempre `Date` com `timestamps: true`
- ❌ Enums como inteiros — use `string` lowercase
- ❌ Índices sem `name` explícito
- ❌ Schema sem `versionKey` quando o documento evolui ao longo do tempo
