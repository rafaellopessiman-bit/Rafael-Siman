...
description: >
  Guia a estrutura de pastas, camadas e padrões de Clean Architecture do atlas_local
  (Python 3.12 + PyMongo/Motor + Groq LLM). Use ao criar módulos Python, revisar
  fronteiras de camada ou convenções de nomenclatura.
paths: src/**/*.py
---

# Skill: Python Clean Architecture — atlas_local

Descreve a estrutura modular do projeto `atlas_local`, as fronteiras entre camadas e os padrões de implementação esperados em cada módulo.
...
main.py  →  handlers (main_cli_*.py)
         ...
         storage  (document_store)
              ↓
         core   (llm_client, output, schemas, config)
              ↓
         exceptions.py
```
...
Segue Dependency Rule: importa apenas de core/ e exceptions.
"""
...
| Regra | Detalhe |
|---|---|
| Configuração | Sempre via `get_settings()` de `src/core/config.py` — nunca `os.environ` direto |
...
```python
# src/core/config.py — uso correto
from src.core.config import get_settings
